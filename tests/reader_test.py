...
def test_get_top_grid(reader):
    top_grid = reader._top_grid(["5 5"])
    assert top_grid == [5, 5]


def test_get_position(reader):
    position = reader._get_position("1 2 N")
...
def test_get_instructions(reader, mocker):
    ...
    instructions = reader._get_instructions(["5 5", "1 2 N", "LMLMLMLMM", "3 3 E", "MMRMMRMRRM"])
